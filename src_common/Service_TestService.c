...
void ProtocolTest2()
{
	...
	for(i = 0; i < 100; i++)
	{
		for(j = 0; j < 50; j++)
		{
			...
			// Create the data transfer packet
			newPacket.a.communicationType = COMM_TYPE_UNICAST;
			newPacket.transmissionInfo.destination = 4; /*(gpioDataRegisters.GPBDAT.bit.SWITCH1 << 3) + (gpioDataRegisters.GPBDAT.bit.SWITCH2 << 2) + 
...
#pragma CODE_SECTION(ProtocolTest3,"FlashCode");
void ProtocolTest3()
{
#if PROFILE_TEST_SERVICE == true
	double executionTime;
...
void Convolution(int* x, int* y, int xLength, int yLength, int* result, int numNodesToUse)
{
	...
	int receivedResults[MAX_NUM_NODES] = {false,false,false,false,false,false,false,false,false};
	MPI_Status status;
	...
	else
	{
		...
		for(n = 0; n < (rank * resultLength) / numNodesToUse; n++)
			result[n] = 0;
		for(n = (rank * resultLength) / numNodesToUse; n < ((rank + 1) * resultLength) / numNodesToUse; n++)
		{
			result[n] = 0;
			for(m = n - yLength; m < n + yLength; m++)
			{
	...
	if(numNodesToUse > 1)
	{
		...
		{	
			...
			for(n = 1; n < numNodesToUse; n++)
				MPI_Irecv((void**)(&(nodeResults[n])),resultLength,MPI_SHORT,n,0,MPI_COMM_WORLD,&(recvRequests[n]));
				...
				for(n = 1; n < numNodesToUse; n++)
				{
					if(!receivedResults[n])
					{
						...
						if(flag)
						{
							...
							for(m = (n * resultLength) / numNodesToUse; m < ((n + 1) * resultLength) / numNodesToUse; m++)
								result[m] = nodeResults[n][m - (n * resultLength) / numNodesToUse];
		...
		else
		{
			...
			MPI_Send(&(result[(rank * resultLength) / numNodesToUse]),
				((rank + 1) * resultLength) / numNodesToUse - (rank * resultLength) / numNodesToUse,
				MPI_SHORT,0,0,MPI_COMM_WORLD);
		}
	}
}
